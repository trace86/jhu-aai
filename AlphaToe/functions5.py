...
def check_win(arr, verbose=False):
    if len(arr) == 4:
...
def getWinner(board, verbose=False):
    board_array = np.array(board)
    wins = []

    for j in range(5):
        winner = check_win(board_array[:, j], verbose=verbose)
        wins.append(winner)

    for i in range(5):
        winner = check_win(board_array[i, :], verbose=verbose)
        wins.append(winner)
    ...
    for each in diags:
        if len(each) >= 4:
            wins.append(check_win(each, verbose=verbose))
...
def getModel():
    numCells = 25 # How many cells in a 3x3 tic-tac-toe board?
